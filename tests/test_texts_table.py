...
WIRED = {"药水效果": PKG_POTION_SRC, "场景触发": PKG_POI_EFFECTS_SRC,
        ...
        "职业机制": [PKG_CLASS_MECH_SRC,
         ...
         #   （两文件均本片首次接入 `_T`；20 + 16 = 36 处替换 / 36 新键，两新分类；零同值复用）
         "职业速查": os.path.join(PKG_CONTENT, "cmds_job.py"),
         "收藏册": os.path.join(PKG_CONTENT, "cmds_collection.py"),
         # ★ C 档 33c（B-2 第 23 片）：wild_king「野王 / 野王宝箱」+ travel「出行提示」
         #   （两文件均本片首次接入 `_T`；23 + 10 = 33 处替换 / 33 新键，三新分类）
         "野王": os.path.join(PKG_CONTENT, "wild_king.py"),
         "野王宝箱": os.path.join(PKG_CONTENT, "wild_king.py"),
         "出行提示": os.path.join(PKG_CONTENT, "travel.py"),
         # ★ C 档 34a（B-2 第 24 片）：talk_actions「对话动作回执」+ race_talent_display
         #   「种族天赋展示行」（两文件均本片首次接入 `_T`；19 + 24 = 43 处替换 / 43 新键，
         #   两新分类；零同值复用）
         "对话动作": os.path.join(PKG_CONTENT, "talk_actions.py"),
         "种族天赋": os.path.join(PKG_CONTENT, "race_talent_display.py"),
         # ★ C 档 34b（B-2 第 25 片）：cmds_event「今日事件」（总览三栏/单图详情/
         #   概率模糊带档位）+ reward「奖励发放」（装备/宠物蛋/缰绳/称号/永久属性/经验金币播报）；
         #   `补给箱` 分类沿用既有 WIRED（同文件 cmds_event.py）。
         "今日事件": os.path.join(PKG_CONTENT, "cmds_event.py"),
         "奖励发放": os.path.join(PKG_CONTENT, "reward.py"),
        ...
        "探索事件": [os.path.join(PKG_CONTENT, "combat_cmds.py"),
         ...
         "社交市场": [PKG_SOCIAL_SRC, os.path.join(PKG_CONTENT, "social_stall.py"),
                  os.path.join(PKG_CONTENT, "persistence", "social.py")],
        ...
        "公共规则": os.path.join(PKG_CONTENT, "cmds_base_rules.py"),
         ...
         # ★ C 档 42a（2026-09-19）：怪物命名（幼崽/爪牙名）——drops 与副本爪牙阵列两处引用面
         "怪物命名": [os.path.join(PKG_CONTENT, "drops.py"), PKG_INSTANCE_SRC],
         ...
         # ★ P-54（2026-09-26）路由回话：引擎 `host/runtime.py::_miss_reply` 改成**必需注入**
         #   `route_miss_text_fn`（未装配 ⇒ 抛 EngineNotConfigured），内容半边 = 包内
         #   `content/apply.py::route_miss_text` 挂在 `config.mount(...)` 上，句子真源
         #   = 本表 `route.miss`（代码零中文，只传槽位 word）⇒ 引用面 = 装配入口本文件。
         #   行为判据（装了 / 未命中回话指向本包真有的声明 / 撤掉即 fail-closed）见
         #   `tests/test_route_miss_text.py`。
         "路由回话": os.path.join(PKG_CONTENT, "apply.py")}
...
def t1_table_selfcheck():
    ...
    check("category 取值符合预期（副本准入 / 副本日志 / 副本面板 / 副本移动 / 签到 / 周常 / 补给箱 / 每日任务 / 武器特效 / 效果名 / 机制名 / 增益名 / 减益名 / 叠层名 / 资源名 / 战斗日志 / 天气名 / 季节名 / 属性名 / 团队特效名 / 条件文案 / 宠物技能描述 / 帮助面板 / 副业图标 / 技能面板 / GM面板 / GM指令 / 公会面板 / 修炼塔 / 角色面板 / 属性面板 / 排行榜 / 种族面板 / 转职 / 技能栏 / 流派 / 技能详情 / 经济面板 / 生活副业 / 炼金 / 烹饪 / 锻造 / 强化 / 宝石 / 符文 / 重锻炼成 / 地图导航 / 任务委托 / 家园地契 / 营地休息 / 声望阵营 / 传送方碑 / 场景交互 / 战斗主循环 / 战斗面板 / 探索事件 / 社交市场 / 社交队伍 / 社交公会 / 世界事件 / 社交拍卖 / 宠物面板 / 宠物喂养 / 宠物管理 / 坐骑面板 / 商店面板 / 商店购买 / 商店出售 / 商店限购 / 背包面板 / 装备面板 / 道具使用 / 称号面板 / NPC面板 / NPC查找 / NPC对话 / 快捷交互 / 移动赶路 / 行会就职 / NPC教习 / 见闻录 / 时间面板 / 时段名 / 任务目标 / 武器礼包 / 战斗结算 / 掉落播报 / 任务列表 / 任务接取 / 任务交付 / 任务进度 / 战力面板 / 编年史 / 副业等待 / 垂钓结算 / 垂钓惊喜 / 采集结算 / 挖掘结算 / 职业速查 / 收藏册 / 野王 / 野王宝箱 / 出行提示 / 对话动作 / 种族天赋 / 今日事件 / 奖励发放 / 成就面板 / 意见反馈 / 药水效果 / 场景触发 / 职业机制 / 事件模板 / 装备描述 / 团队机制 / 元素机制 / 升级提示 / 探索进度 / 公共规则 / 怪物命名 / 路由回话）",
          set(cats) == {"副本准入", "副本日志", "副本面板", "副本移动", "签到", "周常", "补给箱", "每日任务",
                        ...
                        "副业等待", "垂钓结算", "垂钓惊喜", "采集结算", "挖掘结算",
                        # ★ C 档 33b（B-2 第 22 片）：cmds_job「职业速查」/ cmds_collection「收藏册」
                        "职业速查", "收藏册",
                        # ★ C 档 33c（B-2 第 23 片）：wild_king「野王 / 野王宝箱」+ travel「出行提示」
                        "野王", "野王宝箱", "出行提示",
                        # ★ C 档 34a（B-2 第 24 片）：talk_actions「对话动作」+
                        #   race_talent_display「种族天赋」（21 个 _d_* 展示格式化函数）
                        "对话动作", "种族天赋",
                        # ★ C 档 34b（B-2 第 25 片）：cmds_event「今日事件」+ reward「奖励发放」
                        "今日事件", "奖励发放",
                        ...
                        "任务面板",
                        # ★ C 档 42a（2026-09-19）：怪物命名句壳（drops 幼崽/爪牙名 · 副本爪牙名）
                        "怪物命名",
                        # ★ P-54（2026-09-26）：路由回话（引擎 `_miss_reply` 必需注入
                        #   `route_miss_text_fn` ⇒ 内容半边在 `content/apply.py`）
                        "路由回话"}, cats)
