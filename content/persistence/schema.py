...
表结构声明在 `content/data/tables.json`（表序 / 列序 / 类型 / 主键 / 非空 / 默认 /
...
_TABLES_JSON = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "tables.json")
...
def install(db):
    """把 `content/data/tables.json` 的表结构声明注册到**注入的连接骨架**上（宿主工厂调一次；幂等）。

    逐表 `declare`：引擎从同一份声明派生建表语句与补列迁移，并当场 `db.init()`
    （自带幂等；调用方随后的 `init_db()` 是重复无害）。
    """
    repos = declare_file(db, _TABLES_JSON)
    flush_log("persistence.schema：已声明 %d 张表（data/tables.json；db=%s）"
