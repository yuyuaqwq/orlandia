...
| `from .. import content as C`（`C.NPCS` / `C.ALL_WILD` / `C.SIDE_QUESTS` / `C.CLASSES` / `C.resolve` / `C.get_dialogue`） | 本文件 `class _Dom` + `C = _Dom()`（读包内域 JSON） | ★ 命令层**改从包取**的全部分（父任务点名 6 张表里的 5 张：npcs/quests/classes + items 见 event_menu） |
...
★ `_Dom` 的两处形状还原（JSON 只有字符串键 / dict 只剩字典序）
--------------------------------------------------------------
...
2. `ALL_WILD`：真源 `game/core/wild.py:26 ALL_WILD = {**WILD_NPCS, **HIDDEN_NPCS}` 在
   ...
   不在 `ALL_WILD` 里）。包内 npcs 域用 `source=="hidden" and not inst_stage` 精确还原这 63 条
   （快照探针逐键对拍）。
"""
...
def _int_keys(tbl) -> dict:
    """字符串键 → int 键（非整数键原样保留；`content/tables.py:_int_keys` 同款）。"""
    out: dict = {}
...
# ============================================================
# 包内域门面（替身：宿主薄聚合层 `C`）—— 只暴露本文件正文用到的 6 个符号
# ============================================================
class _Dom:
    """`C.NPCS` / `C.ALL_WILD` / `C.SIDE_QUESTS` / `C.CLASSES` / `C.resolve` / `C.get_dialogue`。"""

    def __init__(self):
        self.npcs_raw: dict = _read_domain("npcs")
        self.quests_raw: dict = _read_domain("quests")
        self.dialogues: dict = _read_domain("dialogues")
        # C.NPCS = 城镇/据点 NPC（真源 npcs 域 source=="town"，362 条）
        self.NPCS = {k: v for k, v in self.npcs_raw.items() if v.get("source") == "town"}
        # C.ALL_WILD = 野外 ∪ 隐藏（**不含** 6 条装配期并入的层内 NPC —— 见文件头 ★）
        self.ALL_WILD = {k: v for k, v in self.npcs_raw.items()
                         if v.get("source") == "wild"
                         or (v.get("source") == "hidden" and not v.get("inst_stage"))}
        # C.SIDE_QUESTS = 支线（真源 quests 域 source=="side"，144 条；list 序无关：按 id 取）
        self.SIDE_QUESTS = [v for v in self.quests_raw.values() if v.get("source") == "side"]
        # C.CLASSES：只还原 `evolve_branches` 的 int 档位键（本文件唯一的 CLASSES 读点）
        self.CLASSES = {}
        for _cid, _c in (_read_domain("classes") or {}).items():
            _c = dict(_c)
            if isinstance(_c.get("evolve_branches"), dict):
                _c["evolve_branches"] = _int_keys(_c["evolve_branches"])
            self.CLASSES[_cid] = _c
